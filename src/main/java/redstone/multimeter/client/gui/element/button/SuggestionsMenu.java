...
public class SuggestionsMenu extends AbstractElement {
	...
	public void render(PoseStack poses, int mouseX, int mouseY) {
		if (isShowingSuggestions()) {
			...
			for (int i = 0; i < size && scroll + i < suggestions.size(); i++) {
				renderText(font, poses, suggestions.get(scroll + i), x, y, false, 0xFFFFFFFF);
	...
	private void renderBackground(PoseStack poses) {
		int x = getX();
		int y = getY();
		int width = getWidth();
		...
		renderRect(poses, x, y, width, height, backgroundColor);
	}

	private void renderHighlights(PoseStack poses) {
		int x = getX();
		int width = getWidth();
		int height = font.lineHeight + ENTRY_SPACING;
		...
		if (hovered >= 0) {
			...
			renderRect(poses, x, y, width, height, hoveredColor);
		}
		if (selected >= 0) {
			int y = getY() + (selected - scroll) * height;
			int selectedColor = getSelectedColor();

			renderRect(poses, x, y, width, height, selectedColor);
		}
	}
