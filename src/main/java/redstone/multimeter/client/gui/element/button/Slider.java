...
public class Slider extends AbstractButton {
	...
	public Slider(MultimeterClient client, int x, int y, Supplier<String> message, Supplier<Tooltip> tooltip, Consumer<Double> onSlide, Supplier<Double> valueSupplier, long steps) {
		this(client, x, y, DEFAULT_WIDTH, DEFAULT_HEIGHT, message, tooltip, onSlide, valueSupplier, steps);
	}

	public Slider(MultimeterClient client, int x, int y, int width, int height, Supplier<String> message, Supplier<Tooltip> tooltip, Consumer<Double> valueConsumer, Supplier<Double> valueSupplier, long steps) {
