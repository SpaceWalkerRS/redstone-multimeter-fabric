...
public class Slider extends AbstractButton {
	...
	public boolean mouseClick(double mouseX, double mouseY, int button) {
		...
		if (!consumed && this.isActive() && button == MOUSE_BUTTON_LEFT) {
	...
	public boolean mouseDrag(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
		if (this.isActive() && this.isDraggingMouse() && button == MOUSE_BUTTON_LEFT) {
			this.updateValue(mouseX);
			return true;
		}
	...
	private void setValue(double newValue, boolean updateListener) {
		this.value = MathUtils.clamp(newValue, 0.0D, 1.0D);
