...
public class MeterControlsElement extends AbstractParentElement {
	...
	public MeterControlsElement(MultimeterClient client, int x, int y, int width) {
		...
		this.title = new TextElement(this.client, 0, 0, t -> t.add(Formatting.UNDERLINE + String.format("Edit Meter \'%s\'", meter == null ? "" : meter.getName())).setWithShadow(true));
		this.hideButton = new Button(this.client, 0, 0, 18, 18, () -> meter != null && meter.isHidden() ? "\u25A0" : "\u25A1", () -> Tooltip.of(String.format("%s Meter", meter == null || meter.isHidden() ? "Unhide" : "Hide")), button -> {
			this.client.getMeterGroup().toggleHidden(meter);
			return true;
		});
		this.deleteButton = new Button(this.client, 0, 0, 18, 18, () -> (triedDeleting ? Formatting.RED : Formatting.WHITE) + "X", () -> Tooltip.of("Delete Meter").add(TextUtils.formatKeybindInfo(Keybinds.TOGGLE_METER)), button -> {
			tryDelete();
		...
		this.deleteConfirm = new TextElement(this.client, 0, 0, t -> t.add(Formatting.ITALIC + "Are you sure you want to delete this meter? YOU CANNOT UNDO THIS!").setWithShadow(true));
