...
public class MeterControlsElement extends AbstractParentElement {
	...
	public MeterControlsElement(MultimeterClient client, int x, int y, int width) {
		...
		this.deleteButton = new Button(this.client, 0, 0, 18, 18, () -> Text.literal("X").formatted(triedDeleting ? Formatting.RED : Formatting.WHITE), () -> Tooltip.of("Delete Meter").add(TextUtils.formatKeybindInfo(KeyBindings.TOGGLE_METER)), button -> {
			tryDelete();
