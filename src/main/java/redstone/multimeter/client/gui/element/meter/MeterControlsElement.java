...
public class MeterControlsElement extends AbstractParentElement {
	...
	public MeterControlsElement(int x, int y, int width) {
		...
		this.deleteButton = new BasicButton(0, 0, 18, 18, () -> Texts.literal("X").format(this.triedDeleting ? Formatting.RED : Formatting.WHITE), () -> Tooltips.keybind(Texts.translatable("rsmm.gui.meterControls.delete"), Keybinds.TOGGLE_METER), button -> {
			...
			if (this.triedDeleting && Screen.isShiftDown()) {
	...
	public boolean keyPress(int keyCode) {
		...
		if (this.triedDeleting && keyCode == Keyboard.KEY_ESCAPE) {
			this.undoTryDelete();
			consumed = true;
		}
	...
	private void createControls() {
		...
		pos.addControl("dimension", new TextField(0, 0, 0, 0, Tooltips::empty, dimension -> {
			changePos(meter.getPos().offset(dimension));
		}, () -> this.meter.getPos().getDimension().toString()), SuggestionsProvider.matching(Dimensions.REGISTRY, false));
		...
		for (EventType type : EventType.ALL) {
			...
			Supplier<Tooltip> tooltip = () -> keybind.keyCode == Keyboard.KEY_NONE
				? Tooltips.EMPTY
