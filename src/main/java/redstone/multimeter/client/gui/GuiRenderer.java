...
public class GuiRenderer {
	...
	private GuiRenderer(FontRenderer font) {
		...
		this.textureManager = MultimeterClient.MINECRAFT.textureManager;
	...
	private void innerBlit(Texture t, int x0, int x1, int y0, int y1, float u0, float u1, float v0, float v1, int color) {
		MultimeterClient.MINECRAFT.textureManager.bind(MultimeterClient.MINECRAFT.textureManager.load(t.location));
		GL11.glEnable(GL11.GL_TEXTURE_2D);
		GL11.glEnable(GL11.GL_DEPTH_TEST);
	...
	private void applyScissor(ScissorBox box) {
		if (box == null) {
			...
			int windowHeight = minecraft.height;
	...
	public void renderItem(ItemStack item, int x, int y) {
		...
		this.itemRenderer.renderGuiItem(this.font.font, this.textureManager, item.itemId, item.getMetadata(), item.getSprite(), x, y);
	}
