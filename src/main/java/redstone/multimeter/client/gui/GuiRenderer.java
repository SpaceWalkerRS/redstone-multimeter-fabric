...
public class GuiRenderer {
	...
	private void innerBlit(Texture t, int x0, int x1, int y0, int y1, float u0, float u1, float v0, float v1, int color) {
		...
		builder.vertex(x0, y1, 0).color(r, g, b, a).uv(u0, v1).endVertex();
		builder.vertex(x1, y1, 0).color(r, g, b, a).uv(u1, v1).endVertex();
		builder.vertex(x1, y0, 0).color(r, g, b, a).uv(u1, v0).endVertex();
		builder.vertex(x0, y0, 0).color(r, g, b, a).uv(u0, v0).endVertex();
		builder.end();
