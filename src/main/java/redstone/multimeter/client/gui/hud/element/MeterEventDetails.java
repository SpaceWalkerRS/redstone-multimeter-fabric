...
public class MeterEventDetails extends AbstractElement {
	...
	private void renderEventDetails(PoseStack poses, EventLog event) {
		...
		// background
		hud.renderer.renderRect(poses, x    , y + 1         , width    , height - 2, backgroundColor); // center, left/right outer borders
		hud.renderer.renderRect(poses, x + 1, y             , width - 2, 1         , backgroundColor); // top outer border
		hud.renderer.renderRect(poses, x + 1, y + height - 1, width - 2, 1         , backgroundColor); // bottom outer border

		// inner border
		hud.renderer.renderGradient(poses, x + 1        , y + 2         , 1        , height - 4, borderColor0, borderColor1); // left
		hud.renderer.renderRect    (poses, x + 1        , y + height - 2, width - 2, 1         , borderColor1);               // bottom
		hud.renderer.renderGradient(poses, x + width - 2, y + 2         , 1        , height - 4, borderColor0, borderColor1); // right
		hud.renderer.renderRect    (poses, x + 1        , y + 1         , width - 2, 1         , borderColor0);               // top
		...
		for (int index = 0; index < lines.size(); index++) {
			...
			int lineX = textX + (leftToRight ? 0 : (width - 8) - hud.font.width(line.getColoredString()));
			int lineY = textY + (topToBottom ? 1 : -1) * (index + (topToBottom ? 0 : 1)) * (hud.font.lineHeight + 1) + (topToBottom ? 0 : (height - 4));

			hud.renderer.renderText(poses, line, lineX, lineY, 0xFFFFFFFF);
		}
